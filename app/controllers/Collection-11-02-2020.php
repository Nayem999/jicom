...
class Collection extends MY_Controller
{
      ...
      $this->sales_model->salesDeu($u_id);
       ...
       echo '<div class="col-xs-10">
            ...
            <table class="table table-bordered">
             ...
             <div class="col-xs-10">
            <p>Total Customer information</p>
            <table class="table table-bordered">
                <tbody>
                  <tr>
                  ...
                  <!--<tr>
                    ...
                    <th class="col-xs-7" id="total_customer_deu">'.$this->tec->formatMoney($tdeu).'                     
                     </th>-->
                  </tr>
                   <tr>
    ...
    public function todayCollectionPayment(){
        ...
        /*if($type=='card'){
          $cardData = array(
            'bank_account_id' => $this->input->post('bank'),
            'tran_amount'  => $this->input->post('amount'),       
            'tran_type'    => 1,        
            'tran_date'    => date('Y-m-d H:i:s')   
          );
          $this->site->insertQuery('tranjiction',$cardData);          
        }*/
  ...
  public function bankInfo($type,$bank_id=NULL,$check=NULL){   
      //$suppliers = $this->purchases_model->getSupplierByID($sid);
      
      $banks = $this->site->seleteQuery('bank_account'); 
      ...
      if($type == 'cheque'){
        $output= '<div class="form-group">
            ...
            foreach ($banks as $key => $bank) {
              if($bank_id == $bank->bank_account_id){
                $selected = 'selected';
              }else{
                $selected = '';
              }
              $output .='<option value="'.$bank->bank_account_id.'" '. $selected .'>'.$bank->bank_name .' ('.$bank->account_name.' ) ( '.$bank->account_no.')</option>';
            }
                        
        $output .='</select></div>
                <div class="form-group">
                <label>Cheque No </label>
            <input type="text" name="cheque_no" class="form-control" required="required" value="'. $check .'">
                </div>'; 
        echo $output;
      }else if($type == 'card'){
        ...
        $output .='</select></div>
                <div class="form-group">
                <label>Card No </label>
            <input type="text" name="cheque_no" class="form-control" value="'. $check .'">
                </div>'; 
        echo $output;
      } 
