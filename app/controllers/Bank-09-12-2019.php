...

{
    ...
    function add() { 
		...
		$this->form_validation->set_rules('current_amount', 'Current Amount', 'required');
	...
	public function pendingCheque(){ 	
		$this->data['title'] = 'Pending Cheque'; 
	...
	public function get_pendingCheque(){
         ...
         $this->datatables->join('suppliers', 'suppliers.id=bank_pending.supplier_id');
         $this->datatables->join('bank_account', 'bank_account.bank_account_id=bank_pending.bank_id');
         $this->datatables->join('stores', 'bank_account.store_id=stores.id'); 
	...
	public function get_pending_loan(){
         ...
         $this->datatables->join('payloaner', 'payloaner.id=bank_pending_loan.loan_id');
         $this->datatables->join('bank_account', 'bank_account.bank_account_id=bank_pending_loan.bank_id');
         $this->datatables->join('loaner', 'loaner.id=bank_pending_loan.loner_id'); 
	...
	public function chequeApproed($id){
		...
		$dataAppr = array(
			'type' => $this->input->post('paid_by'),
			);
		
		if($this->input->post('paid_by')=='Approved'){

			if($bankInfo->current_amount > $amount){ 
